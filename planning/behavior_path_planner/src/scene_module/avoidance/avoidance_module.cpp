...
#define DEBUG_PRINT(...) RCLCPP_DEBUG_EXPRESSION(getLogger(), parameters_->print_debug_info, __VA_ARGS__)
...
AvoidanceModule::AvoidanceModule(const std::string& name, rclcpp::Node& node,
...
BT::NodeStatus AvoidanceModule::updateState()
{
  ...
  DEBUG_PRINT("is_plan_running = %d, has_avoidance_target = %d", is_plan_running, has_avoidance_target);
...
AvoidancePlanningData AvoidanceModule::calcAvoidancePlanningData(DebugData& debug) const
{
  ...
  {
    RCLCPP_WARN_THROTTLE(getLogger(), *clock_, 5000, "calcCenterLinePath() must return path which size > 1");
    return data;
  }

  // reference path
  data.reference_path = util::resamplePathWithSpline(center_path, parameters_->resample_interval_for_planning);
  ...
  data.ego_closest_path_index = std::min(nearest_segment_index + 1, data.reference_path.points.size() - 1);

  // arclength from ego pose (used in many functions)
  data.arclength_from_ego =
      util::calcPathArcLengthArray(data.reference_path, 0, data.reference_path.points.size(),
                                   calcSignedArcLength(data.reference_path.points, getEgoPosition(), 0));

  // lanelet info
  data.current_lanelets = util::calcLaneAroundPose(planner_data_->route_handler, reference_pose.pose,
...
ObjectDataArray AvoidanceModule::calcAvoidanceTargetObjects(const lanelet::ConstLanelets& current_lanes,
                                                            ...
                                                            DebugData& debug) const
{
  ...
  const auto objects_candidate =
      util::filterObjectsByVelocity(*planner_data_->dynamic_object, parameters_->threshold_speed_object_is_stopped);
  ...
  const auto expanded_lanelets =
      lanelet::utils::getExpandedLanelets(current_lanes, parameters_->detection_area_left_expand_dist,
                                          parameters_->detection_area_right_expand_dist * (-1.0));
  const auto lane_filtered_objects_index = util::filterObjectIndicesByLanelets(objects_candidate, expanded_lanelets);
  ...
  const auto dist_to_goal = rh->isInGoalRouteSection(expanded_lanelets.back()) ?
  ...
  for (const auto& i : lane_filtered_objects_index)
  {
    ...
    if (overhang_lanelet.id())
    {
      ...
      lanelet::BasicPoint3d overhang_basic_pose(object_data.overhang_pose.position.x,
                                                object_data.overhang_pose.position.y,
                                                object_data.overhang_pose.position.z);
      const bool get_left = isOnRight(object_data) && parameters_->enable_avoidance_over_same_direction;
      const bool get_right = !isOnRight(object_data) && parameters_->enable_avoidance_over_same_direction;

      const auto target_lines = rh->getFurthestLinestring(overhang_lanelet, get_right, get_left,
  ...
  // debug
  {
    ...
    debug_avoidance_msg_array_ptr_ = std::make_shared<AvoidanceDebugMsgArray>(debug_data_.avoidance_debug_msg_array);
    debug.farthest_linestring_from_overhang = std::make_shared<lanelet::ConstLineStrings3d>(debug_linestring);
...
void AvoidanceModule::updateRegisteredRawShiftLines()
{
  ...
  const int margin = 0;
  const auto deadline =
      static_cast<size_t>(std::max(static_cast<int>(avoidance_data_.ego_closest_path_index) - margin, 0));
  ...
  DEBUG_PRINT("ego_closest_path_index = %lu, registered_raw_shift_lines_ size: %lu -> %lu",
...
AvoidLineArray AvoidanceModule::calcShiftLines(AvoidLineArray& current_raw_shift_lines, DebugData& debug) const
{
  /**
...
double AvoidanceModule::getShiftLength(const ObjectData& object, const bool& is_object_on_right,
                                       const double& avoid_margin) const
{
...
AvoidLineArray AvoidanceModule::calcRawShiftLinesFromObjects(const ObjectDataArray& objects) const
{
  ...
  const auto avoid_margin = lat_collision_safety_buffer + lat_collision_margin + 0.5 * vehicle_width;
  ...
  for (auto& o : objects)
  {
    ...
    if (!has_enough_distance)
    {
      if (remaining_distance <= 0.0)
      {
        ...
        // is not zero.
        avoidance_debug_array_false_and_push_back(AvoidanceDebugFactor::REMAINING_DISTANCE_LESS_THAN_ZERO);
        continue;
      }

      // This is the case of exceeding the jerk limit. Use the sharp avoidance ego speed.
      const auto required_jerk =
          path_shifter_.calcJerkFromLatLonDistance(avoiding_shift, remaining_distance, getSharpAvoidanceEgoSpeed());
    ...
    const auto avoiding_distance = has_enough_distance ? nominal_avoid_distance : remaining_distance;

    DEBUG_PRINT(
        "nominal_lateral_jerk = %f, getNominalAvoidanceEgoSpeed() = %f, prepare_distance = %f, "
        "has_enough_distance = %d",
        parameters_->nominal_lateral_jerk, getNominalAvoidanceEgoSpeed(), prepare_distance, has_enough_distance);
    ...
    DEBUG_PRINT(
        ...
        al_return.end_longitudinal, nominal_avoid_distance, avoiding_distance, avoid_margin, nominal_return_distance);
...
AvoidLineArray AvoidanceModule::fillAdditionalInfo(const AvoidLineArray& shift_lines) const
{
  ...
  std::sort(out_points.begin(), out_points.end(),
            [](auto a, auto b) { return a.end_longitudinal < b.end_longitudinal; });
...
void AvoidanceModule::fillAdditionalInfoFromPoint(AvoidLineArray& shift_lines) const
{
  ...
  const auto dist_path_front_to_ego = calcSignedArcLength(path.points, 0, avoidance_data_.ego_closest_path_index);
...
void AvoidanceModule::fillAdditionalInfoFromLongitudinal(AvoidLineArray& shift_lines) const
{
  const auto& path = avoidance_data_.reference_path;
  const auto arclength = util::calcPathArcLengthArray(path);
  const auto path_front_to_ego = calcSignedArcLength(path.points, 0, avoidance_data_.ego_closest_path_index);
 ...
 */
AvoidLineArray AvoidanceModule::combineRawShiftLinesWithUniqueCheck(const AvoidLineArray& base_lines,
                                                                    const AvoidLineArray& added_lines) const
{
...
void AvoidanceModule::generateTotalShiftLine(const AvoidLineArray& avoid_lines, ShiftLineData& shift_line_data) const
{
  const auto& path = avoidance_data_.reference_path;
...
AvoidLineArray AvoidanceModule::extractShiftLinesFromLine(ShiftLineData& shift_line_data) const
{
  ...
  for (size_t i = avoidance_data_.ego_closest_path_index; i < N - 1; ++i)
  {
    ...
    const bool set_shift_line_flag = std::abs(sl.forward_grad.at(i) - sl.backward_grad.at(i)) > CREATE_SHIFT_GRAD_THR;
...
AvoidLineArray AvoidanceModule::mergeShiftLines(const AvoidLineArray& raw_shift_lines, DebugData& debug) const
{
  ...
  // debug print
  {
    ...
    for (size_t i = 0; i < arc.size(); ++i)
    {
      ...
      ss << "| total: " << setw(5) << sl.at(i) << ") | grad: (" << sg.at(i) << ", " << fg.at(i) << ", " << bg.at(i)
         << ")" << std::endl;
    }
...
std::vector<size_t> AvoidanceModule::calcParentIds(const AvoidLineArray& parent_candidates,
                                                   const AvoidLine& child) const
{
 ...
 * - Remove unnecessary return shift (back to the center line).
 */
AvoidLineArray AvoidanceModule::trimShiftLine(const AvoidLineArray& shift_lines, DebugData& debug) const
{
  if (shift_lines.empty())
  {
    return shift_lines;
...
void AvoidanceModule::alignShiftLinesOrder(AvoidLineArray& shift_lines, const bool recalculate_start_length) const
{
  if (shift_lines.empty())
  {
    return;
  ...
  // sort shift points from front to back.
  std::sort(shift_lines.begin(), shift_lines.end(),
            [](auto a, auto b) { return a.end_longitudinal < b.end_longitudinal; });
...
void AvoidanceModule::trimSmallShiftLine(AvoidLineArray& shift_lines, const double shift_diff_thres) const
{
  ...
  for (size_t i = 1; i < shift_lines_orig.size(); ++i)
  {
    ...
    if (std::abs(shift_diff) < shift_diff_thres)
    {
      ...
      DEBUG_PRINT("i = %lu, relative shift = %f is small. set with relative shift = 0.", i, shift_diff);
    }
    else
    {
...
void AvoidanceModule::trimSimilarGradShiftLine(AvoidLineArray& avoid_lines,
                                               const double change_shift_dist_threshold) const
{
  ...
  for (size_t i = 1; i < avoid_lines_orig.size(); ++i)
  {
    ...
    setEndData(combined_al, al_now.end_shift_length, al_now.end, al_now.end_idx, al_now.end_longitudinal);
    ...
    const auto has_large_length_change = [&]() {
      for (const auto& original : being_merged_points)
      {
        ...
        const auto new_length = combined_al.getGradient() * longitudinal + combined_al.start_shift_length;
        const bool has_large_change = std::abs(new_length - original.end_shift_length) > change_shift_dist_threshold;
...
void AvoidanceModule::trimMomentaryReturn(AvoidLineArray& shift_lines) const
{
  ...
  for (size_t i = 0; i < shift_lines_orig.size(); ++i)
  {
    ...
    const auto sl_prev_length = shift_lines.empty() ? getCurrentLinearShift() : shift_lines.back().end_shift_length;
    ...
    if (!is_reduce_shift)
    {
      ...
      DEBUG_PRINT(
          ...
          i, abs_shift_diff, sl_now.end_shift_length, sl_prev_length, sl_now.end_shift_length * sl_prev_length);
      continue;
    }
    ...
    if (!isZero(sl_next.getRelativeLength()))
    {
      ...
      setStartData(sl_modified, sl_now.end_shift_length, sl_now.start, sl_now.start_idx, sl_now.start_longitudinal);
    ...
    sl_next_modified.start_idx = findPathIndexFromArclength(arclength, sl_next_modified.start_longitudinal);
    sl_next_modified.start = avoidance_data_.reference_path.points.at(sl_next_modified.start_idx).point.pose;
    sl_next_modified.parent_ids = calcParentIds(current_raw_shift_lines_, sl_next_modified);

    // Straight shift point
    if (sl_next_modified.start_idx > sl_now.start_idx)
    {  // the case where a straight route exists.
      auto sl_now_modified = sl_now;
      sl_now_modified.start_shift_length = sl_prev_length;
      setEndData(sl_now_modified, sl_prev_length, sl_next_modified.start, sl_next_modified.start_idx,
                 sl_next_modified.start_longitudinal);
...
void AvoidanceModule::trimSharpReturn(AvoidLineArray& shift_lines) const
{
  ...
  const auto combineShiftLine = [this](const auto& sl_next, const auto& sl_now) {
    auto sl_modified = sl_now;
    setEndData(sl_modified, sl_next.end_shift_length, sl_next.end, sl_next.end_idx, sl_next.end_longitudinal);
    sl_modified.parent_ids = concatParentIds(sl_modified.parent_ids, sl_now.parent_ids);
    return sl_modified;
  };
  ...
  const auto hasViolation = [this](const auto& combined, const auto& combined_src) {
    ...
    for (const auto& sl : combined_src)
    {
      ...
      if (sl.end_shift_length < -0.01 && combined_shift > sl.end_shift_length + VIOLATION_SHIFT_THR)
      {
        return true;
      }
      if (sl.end_shift_length > 0.01 && combined_shift < sl.end_shift_length - VIOLATION_SHIFT_THR)
  ...
  {
    auto sl_now = shift_lines_orig.at(i);
    sl_now.start_shift_length = shift_lines.empty() ? getCurrentLinearShift() : shift_lines.back().end_shift_length;
    ...
    {
      shift_lines.push_back(sl_now);
      DEBUG_PRINT("i = %lu, positive shift. take this one. sl_now.length * sl_now.start_length = %f", i,
    ...
    DEBUG_PRINT("i = %lu, found negative dist. search.", i);
    {
      ...
      for (; i < shift_lines_orig.size(); ++j)
      {
        ...
        const auto long_distance = isZero(sl_combined.end_shift_length) ? nominal_distance : nominal_distance * 5.0;
...
void AvoidanceModule::trimTooSharpShift(AvoidLineArray& avoid_lines) const
{
  ...
  for (size_t i = 0; i < avoid_lines_orig.size(); ++i)
  {
    ...
    for (size_t j = i + 1; j < avoid_lines_orig.size(); ++j)
    {
      ...
      setEndData(al_now, al_next.end_shift_length, al_next.end, al_next.end_idx, al_next.end_longitudinal);
      if (isInJerkLimit(al_now))
      {
        avoid_lines.push_back(al_now);
 ...
 */
void AvoidanceModule::addReturnShiftLineFromEgo(AvoidLineArray& sl_candidates,
                                                AvoidLineArray& current_raw_shift_lines) const
{
  ...
  if (std::fabs(last_sl.end_shift_length) < RETURN_SHIFT_THRESHOLD)
  {
    ...
    {
      if ((current_base_shift > 0.0 && sl.end_shift_length < -ep) ||
    ...
    DEBUG_PRINT("return shift already exists, but they are all candidates. Add return shift for overwrite.");
    last_sl.end = getEgoPose().pose;
    last_sl.end_idx = avoidance_data_.ego_closest_path_index;
  ...
  if (last_sl_distance + 1.0 > remaining_distance)
  {  // tmp: add some small number (+1.0)
  ...
  //
  const double variable_prepare_distance = std::max(nominal_prepare_distance - last_sl_distance, 0.0);
  ...
  {
    const auto scale =
        (remaining_distance - last_sl_distance) / std::max(nominal_avoid_distance + variable_prepare_distance, 0.1);
  ...
  // shift point for return to center line
  {
    ...
    al.start_longitudinal = arclength_from_ego.at(al.start_idx);
    al.end_idx = findPathIndexFromArclength(arclength_from_ego, prepare_distance_scaled + avoid_distance_scaled);
    al.end = avoidance_data_.reference_path.points.at(al.end_idx).point.pose;
    al.end_longitudinal = arclength_from_ego.at(al.end_idx);
...
void AvoidanceModule::generateExtendedDrivableArea(ShiftedPath* shifted_path) const
{
  ...
  for (const auto& current_lane : current_lanes)
  {
    ...
    extended_lanelets.insert(extended_lanelets.end(), extend_from_current_lane.begin(), extend_from_current_lane.end());
    ...
    const lanelet::ConstLanelets next_lanes_from_intersection = std::invoke(
        [&route_handler](const lanelet::ConstLanelet& lane) {
          ...
          for (const auto& prev_lane : prev_lanes)
          {
            ...
            next_lanes.insert(next_lanes.end(), next_lanes_from_prev.begin(), next_lanes_from_prev.end());
          }
          return next_lanes;
        },
        current_lane);

    // 2.1 look for neighbor lane, where end line of the lane is connected to end line of the
    // original lane
    std::copy_if(next_lanes_from_intersection.begin(), next_lanes_from_intersection.end(),
                 std::back_inserter(extended_lanelets), [&current_lane](const lanelet::ConstLanelet& neighbor_lane) {
                   ...
                   return (current_lane.id() != neighbor_lane.id() && is_neighbor_lane);
                 });
  }

  extended_lanelets = util::expandLanelets(extended_lanelets, parameters_->drivable_area_left_bound_offset,
                                           parameters_->drivable_area_right_bound_offset, { "road_border" });

  {
    const auto& p = planner_data_->parameters;
    shifted_path->path.drivable_area = util::generateDrivableArea(
        shifted_path->path, extended_lanelets, p.drivable_area_resolution, p.vehicle_length, planner_data_);
  }
}
...
void AvoidanceModule::modifyPathVelocityToPreventAccelerationOnAvoidance(ShiftedPath& path)
{
  ...
  const auto s_from_ego =
      avoidance_data_.arclength_from_ego.at(target_idx) - avoidance_data_.arclength_from_ego.at(ego_idx);
  ...
  {
    const auto s = avoidance_data_.arclength_from_ego.at(target_idx) - avoidance_data_.arclength_from_ego.at(i);
    const auto t = s / std::max(v0, 1.0);
    if (t < NO_ACCEL_TIME_THR)
    {
      insert_idx = i;
      vmax = std::max(parameters_->min_avoidance_speed_for_acc_prevention,
  ...
  DEBUG_PRINT("s: %f, t: %f, v0: %f, a: %f, vmax: %f, ego_i: %lu, target_i: %lu", s_from_ego, t_from_ego, v0,
...
PathWithLaneId AvoidanceModule::calcCenterLinePath(const std::shared_ptr<const PlannerData>& planner_data,
                                                   const PoseStamped& pose) const
{
  ...
  const auto backward_length = std::max(p.backward_path_length, longest_dist_to_shift_line + extra_margin);

  DEBUG_PRINT("p.backward_path_length = %f, longest_dist_to_shift_line = %f, backward_length = %f",
  ...
  centerline_path =
      util::getCenterLinePath(*route_handler, current_lanes, pose.pose, backward_length, p.forward_path_length, p);
...
boost::optional<AvoidLine> AvoidanceModule::calcIntersectionShiftLine(const AvoidancePlanningData& data) const
{
  ...
  const auto intersection_shift_line = [&]() {
    ...
    const double ego_to_intersection_dist =
        calcSignedArcLength(data.reference_path.points, getEgoPosition(), intersection_point->point.pose.position);
    ...
    p.start = calcBehindPose(intersection_point->point.pose.position, intersection_obstacle_check_dist);
...
BehaviorModuleOutput AvoidanceModule::plan()
{
  ...
  {
    avoidance_path.path = util::resamplePathWithSpline(avoidance_path.path, parameters_->resample_interval_for_output);
  }
...
CandidateOutput AvoidanceModule::planCandidate() const
{
  ...
  if (new_shift_lines)
  {  // clip from shift start index for visualize
...
void AvoidanceModule::addShiftLineIfApproved(const AvoidLineArray& shift_lines)
{
  if (isActivated())
  {
    ...
    {
      right_shift_array_.push_back({ uuid_right_, shift_lines.front().start, shift_lines.back().end });
    }
 ...
 */
void AvoidanceModule::addNewShiftLines(PathShifter& path_shifter, const AvoidLineArray& new_shift_lines) const
{
  ...
  for (const auto& sl : current_shift_lines)
  {
    ...
    {
      DEBUG_PRINT("sl.start_idx = %lu, this sl starts after new proposal. remove this one.", sl.start_idx);
    }
    else
    {
...
boost::optional<AvoidLineArray> AvoidanceModule::findNewShiftLine(const AvoidLineArray& candidates,
                                                                  const PathShifter& shifter) const
{
  ...
  const auto calcJerk = [this](const auto& al) {
    return path_shifter_.calcJerkFromLatLonDistance(al.getRelativeLength(), al.getRelativeLongitudinal(),
  ...
  for (size_t i = 0; i < candidates.size(); ++i)
  {
    ...
    const auto current_shift =
        prev_linear_shift_path_.shift_length.at(findNearestIndex(prev_reference_.points, candidate.end.position));
    ...
    if (std::abs(candidate.end_shift_length - current_shift) > new_point_threshold)
    {
      ...
      {
        DEBUG_PRINT("%s, Failed to find new shift: jerk limit over (%f).", pfx, calcJerk(candidate));
        break;
      }

      DEBUG_PRINT("%s, New shift point is found!!! shift change: %f -> %f", pfx, current_shift,
                  candidate.end_shift_length);
...
PoseStamped AvoidanceModule::getEgoPose() const
{
  return *(planner_data_->self_pose);
}
...
{
  const auto& p = parameters_;
  const auto distance_by_jerk = path_shifter_.calcLongitudinalDistFromJerk(shift_length, parameters_->max_lateral_jerk,
                                                                           getSharpAvoidanceEgoSpeed());
...
void AvoidanceModule::updateData()
{
  ...
  if (prev_linear_shift_path_.path.points.empty())
  {
    ...
    prev_linear_shift_path_.shift_length = std::vector<double>(prev_linear_shift_path_.path.points.size(), 0.0);
  }
...
void AvoidanceModule::updateRegisteredObject(const ObjectDataArray& now_objects)
{
  const auto updateIfDetectedNow = [&now_objects, this](auto& registered_object) {
    ...
    const auto same_id_obj =
        std::find_if(n.begin(), n.end(), [&r_id](const auto& o) { return o.object.object_id == r_id; });
  ...
  const auto isAlreadyRegistered = [this](const auto& n_id) {
    ...
    return std::any_of(r.begin(), r.end(), [&n_id](const auto& o) { return o.object.object_id == n_id; });
  };
...
void AvoidanceModule::CompensateDetectionLost(ObjectDataArray& now_objects) const
{
  ...
  const auto isDetectedNow = [&](const auto& r_id) {
    const auto& n = now_objects;
    return std::any_of(n.begin(), n.end(), [&r_id](const auto& o) { return o.object.object_id == r_id; });
  };
...
bool AvoidanceModule::isTargetObjectType(const PredictedObject& object) const
{
  ...
  const auto is_object_type = ((t == ObjectClassification::CAR && parameters_->avoid_car) ||
...
TurnSignalInfo AvoidanceModule::calcTurnSignalInfo(const ShiftedPath& path) const
{
  ...
  const double turn_signal_shift_length_threshold = planner_data_->parameters.turn_signal_shift_length_threshold;
  const double turn_signal_search_time = planner_data_->parameters.turn_signal_search_time;
  const double turn_signal_minimum_search_distance = planner_data_->parameters.turn_signal_minimum_search_distance;
  ...
  const double ego_vehicle_offset = planner_data_->parameters.vehicle_info.max_longitudinal_offset_m;
  ...
  const auto ego_front_to_shift_start =
      calcSignedArcLength(path.path.points, getEgoPosition(), blinker_start_pose.position) - ego_vehicle_offset;
...
void AvoidanceModule::setDebugData(const PathShifter& shifter, const DebugData& debug)
{
  ...
  const auto add = [this](const MarkerArray& added) { tier4_autoware_utils::appendMarkerArray(added, &debug_marker_); };

  const auto addAvoidLine = [&](const AvoidLineArray& al_arr, const auto& ns, auto r, auto g, auto b, double w = 0.1) {
    add(createAvoidLineMarkerArray(al_arr, ns, r, g, b, w));
  };

  const auto addShiftLine = [&](const ShiftLineArray& sl_arr, const auto& ns, auto r, auto g, auto b, double w = 0.1) {
  ...
  add(createOverhangFurthestLineStringMarkerArray(*debug.farthest_linestring_from_overhang,
                                                  "farthest_linestring_from_overhang", 1.0, 0.0, 1.0));
  ...
  addAvoidLine(debug.trim_similar_grad_shift_second, "c_4_trim_similar_grad_shift", 0.97, 0.32, 0.91);
...
void AvoidanceModule::updateAvoidanceDebugData(std::vector<AvoidanceDebugMsg>& avoidance_debug_msg_array) const
{
  ...
  {
    const bool is_info_old_ = (clock_->now() - debug_avoidance_initializer_for_shift_line_time_).seconds() > 0.1;
    if (!is_info_old_)
    {
      debug_data_avoidance.insert(debug_data_avoidance.end(), debug_avoidance_initializer_for_shift_line_.begin(),
