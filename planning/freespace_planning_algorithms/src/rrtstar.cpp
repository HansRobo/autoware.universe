...
{
  return rrtstar_core::Pose{ pose_msg.position.x, pose_msg.position.y, tf2::getYaw(pose_msg.orientation) };
}
...
{
RRTStar::RRTStar(const PlannerCommonParam& planner_common_param, const VehicleShape& original_vehicle_shape,
                 const RRTStarParam& rrtstar_param)
  : AbstractPlanningAlgorithm(planner_common_param,
                              VehicleShape{ original_vehicle_shape.length + 2 * rrtstar_param.margin,
...
bool RRTStar::makePlan(const geometry_msgs::msg::Pose& start_pose, const geometry_msgs::msg::Pose& goal_pose)
{
  ...
  const rrtstar_core::Pose hi{ costmap_.info.resolution * costmap_.info.width,
                               costmap_.info.resolution * costmap_.info.height, M_PI };
  ...
  auto algo = rrtstar_core::RRTStar(x_start, x_goal, rrtstar_param_.neighbor_radius, collision_check_resolution,
                                    is_informed, cspace);
  while (true)
  {
...
void RRTStar::setRRTPath(const std::vector<rrtstar_core::Pose>& waypoints)
{
  ...
  for (size_t i = 0; i < waypoints.size(); ++i)
  {
    ...
    else
    {
      ...
      const double inpro = cos(pt_now.yaw) * (pt_next.x - pt_now.x) + sin(pt_now.yaw) * (pt_next.y - pt_now.y);
